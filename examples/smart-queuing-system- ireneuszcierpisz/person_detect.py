...
class PersonDetect:
    ...
    def predict(self, image):
        ...
        self.exec_network.start_async(
            request_id=0, inputs={self.input_name: image4infer}
        )
        ...
        coords = self.preprocess_outputs(
            output, image
        )  # return coordinates of bboxes in the frame
    ...
    def preprocess_outputs(self, outputs, image):
        ...
        for bbox in outputs[0][0]:
            if bbox[2] >= self.threshold:
                ...
                coords.append(
                    (xmin, ymin, xmax, ymax)
                )  # append coordinates of bounding box to coords list
